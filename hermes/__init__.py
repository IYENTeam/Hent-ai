...
EMOTION_RULES: list[tuple[str, tuple[re.Pattern[str], ...]]] = [
    ...
    (
        ...
        (
            ...
            re.compile(
                r"완료|성공|통과|해결|고쳤|수정.*완료|빌드.*성공|테스트.*통과|잘 ?됐|문제.*없",
                re.I,
            ),
        ),
    ),
    (
        ...
        (
            ...
            re.compile(
                r"확인.*필요|불확실|잘 ?모르|애매|이해가 안|의미가|어떤.*의미|모호|추가.*정보",
                re.I,
            ),
        ),
    ),
    (
        ...
        (
            ...
            re.compile(
                r"in progress|checking|processing|deploying|testing|verifying|reviewing|reading", re.I
            ),
            re.compile(
                r"분석|조사|확인|살펴|디버깅|검토|읽[어고]|찾[아고]|작업 ?중|처리 ?중|검사",
                re.I,
            ),
        ),
    ),
    (
        ...
        (
            ...
            re.compile(
                r"네[,.]?|알겠|이해했|시작하겠|바로|확인했|말씀대로|지시.*따[르라]|접수",
                re.I,
            ),
        ),
    ),
