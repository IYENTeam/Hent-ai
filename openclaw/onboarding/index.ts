...
export function registerOnboarding(
  ...
  // Single explicit command trigger. No NLP, no LLM, no ambiguity.
  // Onboarding starts ONLY when the user types this exact command.
  const COMMAND_TRIGGER = /^\/hent-ai-onboarding\s*$/i;

  function shouldStartOnboarding(trimmed: string): boolean {
    return COMMAND_TRIGGER.test(trimmed);
  }
